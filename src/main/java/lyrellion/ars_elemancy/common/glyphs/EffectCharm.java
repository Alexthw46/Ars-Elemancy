...
import lyrellion.ars_elemancy.mixin.FoxInvoker;
import lyrellion.ars_elemancy.util.EntityCarryMEI;
...
import static lyrellion.ars_elemancy.registry.ModPotions.ENTHRALLED;
        ...
        if (shooter instanceof Player player && world instanceof ServerLevel level) {
            if (rayTraceResult.getEntity() instanceof Mob mob) {
                ...
                if (mob instanceof Enemy || (mob instanceof NeutralMob && (!(mob instanceof Animal a && a.canFallInLove())))) {
                    ...
                    if (mob.getMaxHealth() < GENERIC_INT.get() || player.getUUID().equals(ArsElemancy.Dev)) {
